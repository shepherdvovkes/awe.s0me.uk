...
// Show API connections endpoint
app.post('/api/show-api-connections', (req, res) => {
    try {
        const { showApiConnections } = require('./command_processor');
        const response = showApiConnections();
        res.json({ output: response });
        
    } catch (error) {
        console.error('API connections error:', error);
        res.status(500).json({ error: 'Failed to show API connections: ' + error.message });
    }
});
...
httpsServer.listen(PORT, () => {
    ...
    console.log(`   POST /api/show-api-connections - Show API connections`);
